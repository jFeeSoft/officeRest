...
export class TaskService {
    ...
    private convertDateFromClient(task: ITask): ITask {
        const copy: ITask = Object.assign({}, task, {
            dateFrom: task.dateFrom != null && task.dateFrom.isValid() ? task.dateFrom.format(DATE_FORMAT) : null,
            dateTo: task.dateTo != null && task.dateTo.isValid() ? task.dateTo.format(DATE_FORMAT) : null
        });
        return copy;
    }
    ...
    private convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
        res.body.forEach((task: ITask) => {
            task.dateFrom = task.dateFrom != null ? moment(task.dateFrom) : null;
            task.dateTo = task.dateTo != null ? moment(task.dateTo) : null;
        });
        return res;
    }
}
